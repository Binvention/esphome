...
void DAC3100::setup() {
  ...
  //DAC takes a while to start so wait a while before starting so check and see if its ready yet
  //otherwise try again on loop fail after 10. 
  this->irq_pin_->setup();
  this->reset_pin_->setup();
  this->start_attempt_ = 1;
  this->dac_ready_ = false;
  this->reset_dac_();
}
...
void DAC3100::config_dac_(){
  if(this->start_attempt_ < this->max_attempts_){
    this->start_attempt_++;
    if(this->write_byte(DAC3100_PAGE_CTRL, 0x00)) {
      ...
      //give some time before marking as ready
      this->set_timeout(20,[this](){this->dac_ready_=true;});
    }
    else {
    ...
    //mark failed
    ESP_LOGE(TAG, "Failed to communicate with DAC");
    this->mark_failed();
    this->start_attempt_++;
  }
}
...
void DAC3100::reset_dac_(){
  ...
  this->set_timeout(100,[this](){
    ...
    this->set_timeout(500,[this](){this->config_dac_();});
  });
}
